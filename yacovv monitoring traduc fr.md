...
Dans une première approche, nous avons testé une mesure capacitive de plusieurs hauteurs sur la colonne d'eau.
La technologie de mesure consiste en un câble isolé qui est guidé dans la colonne d'eau du manomètre, ainsi que d'autres câbles isolés qui sont enroulés sur le manomètre autour de l'emplacement des valeurs de pression à détecter.
...
- Fréquence de ventilation: La fréquence de respiration peut être calculée à partir de la fréquence du pendule [fréquence de balayage ?] de la colonne d'eau dans le manomètre. Une alarme peut être déclenchée via des valeurs min et max dans la surveillance si la fréquence respiratoire est inférieure ou dépassée.

[Video avec détection de 2 valeurs de pression](https://youtu.be/_2SUDyStXJM)

Une mesure capacitive permettrait également de mesurer la hauteur absolue du liquide dans le manomètre, mais cela nécessiterait un étalonnage complexe du système. De plus, les points de mesure fixes peuvent être facilement déplacés sur le manomètre pour ajuster les valeurs. Si, par exemple, la pression de ventilation d'un patient doit être augmentée, il suffit de déplacer le point de mesure vers le haut.
